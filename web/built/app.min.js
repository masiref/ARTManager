function checkTreeChildNodes(id,node){var children=node.nodes;if(children)for(var i=0;i<children.length;i++){var childNode=children[i],nodeId=childNode.nodeId;$(id).treeview("checkNode",nodeId)}}function uncheckTreeChildNodes(id,node){var children=node.nodes;if(children)for(var i=0;i<children.length;i++){var childNode=children[i],nodeId=childNode.nodeId;$(id).treeview("uncheckNode",nodeId)}}function uncheckTreeParentNode(id,node){var parent=$(id).treeview("getParent",node),nodeId=parent.nodeId;void 0!==nodeId&&($(id).treeview("uncheckNode",[nodeId,{silent:!0}]),uncheckTreeParentNode(id,parent))}function refreshObjectMapSubtitle(count){var subtitle="";subtitle+=1>=count?"There is ":"There are ",subtitle+=0===count?"no ":'<span class="badge">'+count+"</span>",subtitle+=" object map"+(count>1?"s":""),$("#object-maps-count").html(subtitle)}function saveObjectMap(applicationId){$.ajax({type:"POST",url:Routing.generate("app_add_application_object_map_ajax",{id:applicationId}),data:$("#form-add-object-map").serialize()}).done(function(data){if(data.error)swal("Object map not added !",data.error,"error");else{var objectMap=JSON.parse(data.objectMap),id=objectMap.id,name=objectMap.name,panel=data.panel;$("#object-maps-row").prepend(panel),$("#delete-object-map-"+id).click(function(event){event.preventDefault(),id=$(this).data("id"),name=$(this).data("name"),deleteObjectMap(id,name)}).tooltip(),$("#open-object-map-"+id).tooltip(),refreshPageSubtitle(0,id);var objectMapsCount=$("[id^=panel-object-map-]").length;refreshObjectMapSubtitle(objectMapsCount),swal(name+" added !","Your object map has been added.","success"),$("#tree-object-map-"+id+"-loader").removeClass("three-quarters-loader"),$("#form-add-object-map")[0].reset(),$("#modal-add-object-map").modal("hide")}})}function deleteObjectMap(id,name){swal({title:"Delete "+name+" ?",text:"You will not be able to recover this object map !",type:"warning",showCancelButton:!0,confirmButtonColor:"#DD6B55",confirmButtonText:"Yes, delete it !",closeOnConfirm:!1},function(){$.ajax({type:"POST",url:Routing.generate("app_delete_application_object_map_ajax",{id:id})}).done(function(data){data.error?swal(name+" not deleted !",data.error,"error"):($("#panel-object-map-"+id).css("visibility","hidden").animate({height:0,width:0},300,function(){var container=$(this).parent();$(this).remove(),container.remove();var objectPropertiesPanel=$("#panel-object-properties");void 0!==objectPropertiesPanel&&objectPropertiesPanel.remove();var objectMapsCount=$("[id^=panel-object-map-]").length;refreshObjectMapSubtitle(objectMapsCount)}),swal(name+" deleted !","Your object map has been deleted.","success"))})})}function refreshObjectMap(id,collapse,checkbox){$.ajax({type:"POST",url:Routing.generate("app_application_get_object_map_tree_ajax",{id:id})}).done(function(data){showObjectMapTree(data,collapse,checkbox),revealObjectMapSelectedNode(id)})}function refreshObjectMaps(applicationId,collapse){$(document).ready(function(){$.ajax({type:"POST",url:Routing.generate("app_application_get_object_map_trees_ajax",{id:applicationId})}).done(function(data){showObjectMapTrees(data,collapse,!1)})})}function hideObjectMapPanel(objectMapId){var objectMapHtmlId="#tree-object-map-"+objectMapId;$(objectMapHtmlId+"-loader").addClass("three-quarters-loader"),$(objectMapHtmlId).hide()}function showObjectMapPanel(objectMapId){var objectMapHtmlId="#tree-object-map-"+objectMapId;$(objectMapHtmlId+"-loader").removeClass("three-quarters-loader"),$(objectMapHtmlId).show()}function showObjectMapTrees(data,collapse,checkbox){jQuery.each(data,function(i,val){var id=i.substring(i.lastIndexOf("-")+1),htmlId="#"+i;$(htmlId).treeview({data:val,showBorder:!1,showCheckbox:checkbox,onNodeSelected:function(event,data){showObjectProperties(data)},onNodeUnselected:function(){hideObjectPropertiesPanelBodyAndFooter(),clearAddObjectDataAttributes()},onNodeChecked:function(event,data){checkTreeChildNodes(htmlId,data),refreshCheckedObjectMapObjectsCount(id)},onNodeUnchecked:function(event,data){uncheckTreeChildNodes(htmlId,data),uncheckTreeParentNode(htmlId,data),refreshCheckedObjectMapObjectsCount(id)}}),collapse&&$(htmlId).treeview("collapseAll",{silent:!0}),revealObjectMapSelectedNode(id),showObjectMapPanel(id)})}function showObjectMapTree(data,collapse,checkbox){showObjectMapTrees(data,collapse,checkbox)}function showObjectMapTreeWithSelectedPage(pageId){$.ajax({type:"POST",url:Routing.generate("app_application_get_object_map_tree_with_selected_page_ajax",{id:pageId})}).done(function(data){showObjectMapTree(data,!0,!0)})}function showObjectMapTreeWithSelectedObject(objectId){$.ajax({type:"POST",url:Routing.generate("app_application_get_object_map_tree_with_selected_object_ajax",{id:objectId})}).done(function(data){showObjectMapTree(data,!0,!0)})}function revealObjectMapSelectedNode(objectMapId){var objectMapHtmlId="#tree-object-map-"+objectMapId,selectedNode=$(objectMapHtmlId).treeview("getSelected")[0];selectedNode&&$(objectMapHtmlId).treeview("revealNode",selectedNode)}function refreshPageSubtitle(count,objectMapId){var subtitle="";subtitle+=1>=count?"There is ":"There are ",subtitle+=0===count?"no ":'<span class="badge">'+count+"</span>",subtitle+=" page"+(count>1?"s":"")+" in the object map",$("#page-count-"+objectMapId).html(subtitle)}function showAddPageForm(id,name,description){$("#new-page-object-map-name").html(name),$("#save-page").data("object-map-id",id),$("#save-page").data("object-map-name",name),$("#new-page-object-map-description").html(description),$("#save-page").data("object-map-description",description),$("#modal-add-page").modal("show")}function savePage(objectMapId,objectMapName){var objectMapHtmlId="#tree-object-map-"+objectMapId,selectedPage=$(objectMapHtmlId).treeview("getSelected"),parentPageId=-1;if(1===selectedPage.length){var parentPage=selectedPage[0],href=parentPage.href,id=href.substring(href.lastIndexOf("-")+1),type=href.substring(href.indexOf("-")+1,href.lastIndexOf("-"));"page"===type&&(parentPageId=id)}$.ajax({type:"POST",url:Routing.generate("app_add_application_object_map_page_ajax",{id:objectMapId,parentId:parentPageId}),data:$("#form-add-page").serialize()}).done(function(data){if(data.error)swal("Page not added !",data.error,"error");else{var name=(data.id,data.name),pagesCount=data.pagesCount;hideObjectMapPanel(objectMapId),$(objectMapHtmlId).treeview({data:data.treeObjectMap,showBorder:!1,showCheckbox:!0,onNodeSelected:function(event,data){showObjectProperties(data)},onNodeUnselected:function(){hideObjectPropertiesPanelBodyAndFooter(),clearAddObjectDataAttributes()},onNodeChecked:function(event,data){checkTreeChildNodes(objectMapHtmlId,data),refreshCheckedObjectMapObjectsCount(objectMapId)},onNodeUnchecked:function(event,data){uncheckTreeChildNodes(objectMapHtmlId,data),uncheckTreeParentNode(objectMapHtmlId,data),refreshCheckedObjectMapObjectsCount(objectMapId)}}),$(objectMapHtmlId).treeview("collapseAll",{silent:!0}),showObjectMapPanel(objectMapId);var selectedNode=$(objectMapHtmlId).treeview("getSelected")[0];showObjectProperties(selectedNode),$(objectMapHtmlId).treeview("revealNode",selectedNode),refreshPageSubtitle(pagesCount,objectMapId),swal(name+" added to "+objectMapName+" !","Your page has been added.","success"),$("#form-add-page")[0].reset(),$("#modal-add-page").modal("hide")}})}function showPageTypeBlock(){$("#page-type-block").show()}function hidePageTypeBlock(){$("#page-type-block").hide()}function showPagePathBlock(){$("#page-path-block").show()}function hidePagePathBlock(){$("#page-path-block").hide()}function refreshObjectSubtitle(count,objectMapId){var subtitle="";subtitle+=1>=count?"There is ":"There are ",subtitle+=0===count?"no ":'<span class="badge">'+count+"</span>",subtitle+=" object"+(count>1?"s":"")+" in the object map",$("#object-count-"+objectMapId).html(subtitle)}function showAddObjectForm(id,name,description){$("#new-object-page-name").html(name),$("#save-object").data("page-id",id),$("#save-object").data("page-name",name),$("#new-object-page-description").html(description),$("#save-object").data("page-description",description),$("#modal-add-object").modal("show")}function saveObject(pageId,pageName){$.ajax({type:"POST",url:Routing.generate("app_add_application_object_map_page_object_ajax",{id:pageId}),data:$("#form-add-object").serialize()}).done(function(data){if(data.error)swal("Object not added !",data.error,"error");else{var name=(data.id,data.name),objectsCount=data.objectsCount,objectMapId=data.objectMapId,objectMapHtmlId="#tree-object-map-"+objectMapId;hideObjectMapPanel(objectMapId),$(objectMapHtmlId).treeview({data:data.treeObjectMap,showBorder:!1,showCheckbox:!0,onNodeSelected:function(event,data){showObjectProperties(data)},onNodeUnselected:function(){hideObjectPropertiesPanelBodyAndFooter(),clearAddObjectDataAttributes()},onNodeChecked:function(event,data){checkTreeChildNodes(objectMapHtmlId,data),refreshCheckedObjectMapObjectsCount(objectMapId)},onNodeUnchecked:function(event,data){uncheckTreeChildNodes(objectMapHtmlId,data),uncheckTreeParentNode(objectMapHtmlId,data),refreshCheckedObjectMapObjectsCount(objectMapId)}}),$(objectMapHtmlId).treeview("collapseAll",{silent:!0}),showObjectMapPanel(objectMapId);var selectedNode=$(objectMapHtmlId).treeview("getSelected")[0];showObjectProperties(selectedNode),$(objectMapHtmlId).treeview("revealNode",selectedNode),$(objectMapHtmlId).treeview("expandNode",selectedNode),refreshObjectSubtitle(objectsCount,objectMapId),swal(name+" added to "+pageName+" !","Your object has been added.","success"),$("#form-add-object")[0].reset(),$("#modal-add-object").modal("hide")}})}function setAddObjectDataAttributes(id,name,description){$("#add-object").data("page-id",id),$("#add-object").data("page-name",name),$("#add-object").data("page-description",description)}function clearAddObjectDataAttributes(){$("#add-object").removeData("page-id"),$("#add-object").removeData("page-name"),$("#add-object").removeData("page-description")}function showObjectTypeBlock(){$("#object-type-block").show()}function hideObjectTypeBlock(){$("#object-type-block").hide()}function deleteObjects(objectMapId){var objectMapHtmlId="#tree-object-map-"+objectMapId;swal({title:"Delete selected objects ?",text:"You will not be able to recover them !",type:"warning",showCancelButton:!0,confirmButtonColor:"#DD6B55",confirmButtonText:"Yes, delete them !",closeOnConfirm:!1},function(){var objects=$(objectMapHtmlId).treeview("getChecked");$.ajax({type:"POST",url:Routing.generate("app_application_object_map_objects_delete_ajax",{id:objectMapId}),data:{objects:objects,selectedNode:$(objectMapHtmlId).treeview("getSelected")[0]}}).done(function(data){if(data.error)swal("Selected objects were not deleted !",data.error,"error");else{var count=data.count,objectsCount=data.objectsCount,pagesCount=data.pagesCount,objectMapId=data.objectMapId;hideObjectMapPanel(objectMapId),$(objectMapHtmlId).treeview({data:data.treeObjectMap,showBorder:!1,showCheckbox:!0,onNodeSelected:function(event,data){showObjectProperties(data)},onNodeUnselected:function(){hideObjectPropertiesPanelBodyAndFooter(),clearAddObjectDataAttributes()},onNodeChecked:function(event,data){checkTreeChildNodes(objectMapHtmlId,data),refreshCheckedObjectMapObjectsCount(objectMapId)},onNodeUnchecked:function(event,data){uncheckTreeChildNodes(objectMapHtmlId,data),uncheckTreeParentNode(objectMapHtmlId,data),refreshCheckedObjectMapObjectsCount(objectMapId)}}),$(objectMapHtmlId).treeview("collapseAll",{silent:!0}),showObjectMapPanel(objectMapId),hideObjectPropertiesPanelBodyAndFooter(),refreshObjectSubtitle(objectsCount,objectMapId),refreshPageSubtitle(pagesCount,objectMapId),refreshCheckedObjectMapObjectsCount(objectMapId),swal("Selected objects deleted !","You have deleted "+count+" object"+(count>1?"s":""),"success")}})})}function showObjectProperties(treeNode){if(treeNode){$("#panel-body-object-properties").show(),$("#panel-footer-object-properties").show(),$("#object-properties-loader").show();var href=treeNode.href,id=href.substring(href.lastIndexOf("-")+1),type=href.substring(href.indexOf("-")+1,href.lastIndexOf("-"));switch($("#object-icon").removeClass("fontello-icon-puzzle"),type){case"page":showPageTypeBlock(),showPagePathBlock(),hideObjectTypeBlock(),hideObjectIdentifierBlock(),$.ajax({type:"POST",url:Routing.generate("app_application_get_object_map_page_ajax",{id:id})}).done(function(data){$("#object-properties-loader").hide();var page=data.page,name=page.name,description=page.description,pageType=page.pageType,path=page.path,createdAt=page.createdAt;$("#object-icon").removeClass().addClass(pageType.icon),$("#object-name").editable("option","pk",id),$("#object-name").editable("option","url",Routing.generate("app_application_object_map_update_page_name_ajax",{id:id})),$("#object-description").editable("option","pk",id),$("#object-description").editable("option","url",Routing.generate("app_application_object_map_update_page_description_ajax",{id:id})),$("#page-type").editable("option","pk",id),$("#page-type").editable("option","url",Routing.generate("app_application_object_map_update_page_type_ajax",{id:id})),$("#page-path").editable("option","pk",id),$("#page-path").editable("option","url",Routing.generate("app_application_object_map_update_page_path_ajax",{id:id})),$("#object-name").editable("setValue",name,!1),$("#object-description").editable("setValue",description,!1),$("#page-type").editable("setValue",pageType.id,!1),$("#page-path").editable("setValue",path,!1),$("#object-creation-date").html(createdAt),setAddObjectDataAttributes(id,name,description)});break;case"object":hidePageTypeBlock(),hidePagePathBlock(),showObjectTypeBlock(),showObjectIdentifierBlock(),$.ajax({type:"POST",url:Routing.generate("app_application_get_object_map_page_object_ajax",{id:id})}).done(function(data){$("#object-properties-loader").hide();var object=data.object,name=object.name,description=object.description,createdAt=object.createdAt,objectType=object.objectType,objectIdentifier=object.objectIdentifier,objectIdentifierType=null;null!==objectIdentifier&&(objectIdentifierType=objectIdentifier.objectIdentifierType),$("#object-icon").removeClass().addClass(object.objectType.icon),$("#object-name").editable("option","pk",id),$("#object-name").editable("option","url",Routing.generate("app_application_object_map_page_update_object_name_ajax",{id:id})),$("#object-description").editable("option","pk",id),$("#object-description").editable("option","url",Routing.generate("app_application_object_map_page_update_object_description_ajax",{id:id})),$("#object-type").editable("option","pk",id),$("#object-type").editable("option","url",Routing.generate("app_application_object_map_page_update_object_type_ajax",{id:id})),$("#object-identifier-type").editable("option","pk",id),$("#object-identifier-type").editable("option","url",Routing.generate("app_application_object_map_page_update_object_identifier_type_ajax",{id:id})),$("#object-identifier-value").editable("option","pk",id),$("#object-identifier-value").editable("option","url",Routing.generate("app_application_object_map_page_update_object_identifier_value_ajax",{id:id})),$("#object-name").editable("setValue",name,!1),$("#object-description").editable("setValue",description,!1),$("#object-type").editable("setValue",objectType.id,!1),null!==objectIdentifier?($("#object-identifier-value").editable("setValue",objectIdentifier.value,!1),null!==objectIdentifierType?$("#object-identifier-type").editable("setValue",objectIdentifierType.id,!1):$("#object-identifier-type").editable("setValue","",!1)):($("#object-identifier-type").editable("setValue","",!1),$("#object-identifier-value").editable("setValue","",!1)),$("#object-creation-date").html(createdAt)});break;default:$("#object-icon").removeClass().addClass("fontello-icon-puzzle")}}else hideObjectPropertiesPanelBodyAndFooter()}function hideObjectPropertiesPanelBodyAndFooter(){$("#panel-body-object-properties").hide(),$("#panel-footer-object-properties").hide()}function refreshCheckedObjectMapObjectsCount(objectMapId){var checkedObjects=$("#tree-object-map-"+objectMapId).treeview("getChecked"),checkedObjectsCount=checkedObjects.length;$("#checked-objects-count").html(checkedObjectsCount),checkedObjectsCount>0?$("#delete-checked-objects").removeClass("disabled"):$("#delete-checked-objects").addClass("disabled")}function showObjectIdentifierBlock(){$("#object-identifier-block").show()}function hideObjectIdentifierBlock(){$("#object-identifier-block").hide()}function refreshObjectTypeIcon(icon){$("#object-icon").removeClass().addClass(icon)}function refreshProjectSubtitle(count){var subtitle="";subtitle+=1>=count?"There is ":"There are ",subtitle+=0===count?"no ":'<span class="badge">'+count+"</span>",subtitle+=" project"+(count>1?"s":""),$("#projects-count").html(subtitle)}function saveProject(){$.ajax({type:"POST",url:Routing.generate("app_add_project_ajax"),data:$("#form-add-project").serialize()}).done(function(data){if(data.error)swal("Project not added !",data.error,"error");else{var project=JSON.parse(data.project),id=project.id,name=project.name,panel=(project.description,data.panel);$("#projects-row").prepend(panel),$("#delete-project-"+id).click(function(event){event.preventDefault();var id=$(this).data("id"),name=$(this).data("name");deleteProject(id,name)}).tooltip(),$("#add-application-"+id).tooltip(),$("#add-application-"+id).click(function(event){event.preventDefault();var id=$(this).data("project-id"),name=$(this).data("project-name"),description=$(this).data("project-description");showAddApplicationForm(id,name,description)}),refreshApplicationSubtitle(0,id),$("#applications-"+id).DataTable({searching:!1,paging:!1,info:!1,order:[[1,"asc"]],columnDefs:[{orderable:!1,targets:0}]});var projectsCount=$("[id^=panel-project-]").length;refreshProjectSubtitle(projectsCount),swal(name+" added !","Your project has been added.","success"),$("#form-add-project")[0].reset(),$("#modal-add-project").modal("hide")}})}function deleteProject(id,name){swal({title:"Delete "+name+" ?",text:"You will not be able to recover this project !",type:"warning",showCancelButton:!0,confirmButtonColor:"#DD6B55",confirmButtonText:"Yes, delete it !",closeOnConfirm:!1},function(){$.ajax({type:"POST",url:Routing.generate("app_delete_project_ajax",{id:id})}).done(function(data){data.error?swal(name+" not deleted !",data.error,"error"):($("#panel-project-"+id).css("visibility","hidden").animate({height:0,width:0},300,function(){container=$(this).parent(),$(this).remove(),container.remove(),projectsCount=$("[id^=panel-project-]").length,refreshProjectSubtitle(projectsCount)}),swal(name+" deleted !","Your project has been deleted.","success"))})})}function refreshApplicationSubtitle(count,projectId){var subtitle="";subtitle+=1>=count?"There is ":"There are ",subtitle+=0===count?"no ":'<span class="badge">'+count+"</span>",subtitle+=" application"+(count>1?"s":"")+" in the project",$("#application-count-"+projectId).html(subtitle)}function showAddApplicationForm(id,name,description){$("#new-application-project-name").html(name),$("#new-application-project-description").html(description),$("#save-application").data("project-id",id),$("#save-application").data("project-name",name),$("#save-application").data("project-description",description),$("#save-application").unbind("click"),$("#save-application").click(function(event){event.preventDefault();var projectId=$(this).data("project-id"),projectName=$(this).data("project-name");saveApplication(projectId,projectName)}),$("#modal-add-application").modal("show")}function saveApplication(projectId,projectName){$.ajax({type:"POST",url:Routing.generate("app_add_application_ajax",{id:projectId}),data:$("#form-add-application").serialize()}).done(function(data){if(data.error)swal("Application not added !",data.error,"error");else{var id=data.id,name=data.name;$("#applications-"+projectId).show();var table=$("#applications-"+projectId).dataTable(),rowNode=$(data.row);table.fnAddTr(rowNode[0]),$("#form-add-application")[0].reset(),$("#modal-add-application").modal("hide"),$("#delete-application-"+id).click(function(event){event.preventDefault();var id=$(this).data("id"),name=$(this).data("name"),projectId=$(this).data("projectId"),projectName=$(this).data("projectName");deleteApplication(id,name,projectId,projectName)});var applicationsCount=$("#applications-"+projectId+" tr").length-1;refreshApplicationSubtitle(applicationsCount,projectId),swal(name+" added to "+projectName+" !","Your application has been added.","success")}})}function deleteApplication(id,name,projectId,projectName){swal({title:"Delete "+name+" from "+projectName+" ?",text:"You will not be able to recover this application !",type:"warning",showCancelButton:!0,confirmButtonColor:"#DD6B55",confirmButtonText:"Yes, delete it !",closeOnConfirm:!1},function(){$.ajax({type:"POST",url:Routing.generate("app_delete_application_ajax",{id:id})}).done(function(data){if(data.error)swal(name+" not deleted !",data.error,"error");else{var oTable=$("#applications-"+projectId).DataTable();oTable.row("#row-application-"+id).remove().draw();var applicationsCount=oTable.data().length;refreshApplicationSubtitle(applicationsCount,projectId),0===applicationsCount&&$("#applications-"+projectId).hide(),swal(name+" deleted !","Your application has been deleted.","success")}})})}function updateStartingPage(testId,pageId,pageName){$.ajax({type:"POST",url:Routing.generate("app_application_test_update_starting_page_ajax",{id:testId}),data:{pageId:pageId}}).done(function(data){data.error?swal("Starting page not updated !",data.error,"error"):($("#starting-page-name").html(pageName),updateBehatScenario(testId))})}function enableStartingPageSelection(){$("#form_startingPage").show(),$(".starting-page-collapse-toggle-icon").show()}function disableStartingPageSelection(){$("#form_startingPage").hide(),$(".starting-page-collapse-toggle-icon").hide()}function triggerStepFormEventListeners(){$("#execute_step_object").change(function(){var testId=$(this).data("test-id");updateStepFormAfterObjectSelection(testId)}),$("#execute_step_action").change(function(){var testId=$(this).data("test-id");updateStepFormAfterActionSelection(testId)})}function showStepFormModal(form,title){$("#modal-execute-step-body").html(form),$("#modal-execute-step-title").html(title),$("#modal-execute-step").modal("show")}function triggerStepEventListeners(id){$("#delete-execute-step-"+id).click(function(event){event.preventDefault();var id=$(this).data("id"),order=$(this).data("order"),testId=$(this).data("test-id");deleteExecuteStep(id,order,testId)}).tooltip(),$("#add-control-step-step-"+id).click(function(event){event.preventDefault();var id=$(this).data("step-id");showAddControlStepForm(id)}).tooltip(),$("#edit-execute-step-"+id).click(function(event){event.preventDefault();var id=$(this).data("id"),testId=$(this).data("test-id");showEditStepForm(id,testId)}).tooltip()}function resetStepFormAndCloseModal(){$("#execute_step_action").parent().remove(),$("#execute_step_parameterDatas").parent().remove(),$("#execute_step_object").val($("#execute_step_object option:first").val()),$("#modal-execute-step").modal("hide")}function showAddStepForm(id){$.ajax({type:"POST",url:Routing.generate("app_get_application_test_step_form_ajax",{testId:id})}).done(function(data){$("#save-execute-step").data("step-id",""),$("#save-execute-step").data("test-id",id),showStepFormModal(data.form,data.modalTitle),triggerStepFormEventListeners()})}function showEditStepForm(id,testId){$.ajax({type:"POST",url:Routing.generate("app_get_application_test_step_form_ajax",{id:id,testId:testId})}).done(function(data){$("#save-execute-step").data("test-id",""),$("#save-execute-step").data("step-id",id),showStepFormModal(data.form,data.modalTitle),triggerStepFormEventListeners()})}function updateStepFormAfterObjectSelection(testId){$("#execute_step_action").parent().remove(),$("#execute_step_parameterDatas").parent().remove(),$.ajax({type:"POST",url:Routing.generate("app_check_application_test_execute_step_ajax",{id:testId}),data:$("#form-execute-step").serialize()}).done(function(data){data.error?swal("Step not updated !",data.error,"error"):($("#form-execute-step").replaceWith($(data.form)),triggerStepFormEventListeners())})}function updateStepFormAfterActionSelection(testId){$("#execute_step_parameterDatas").parent().remove(),$.ajax({type:"POST",url:Routing.generate("app_check_application_test_execute_step_ajax",{id:testId}),data:$("#form-execute-step").serialize()}).done(function(data){data.error?swal("Step not updated !",data.error,"error"):data.form&&($("#form-execute-step").replaceWith($(data.form)),triggerStepFormEventListeners())})}function saveStep(testId){$.ajax({type:"POST",url:Routing.generate("app_add_application_test_execute_step_ajax",{id:testId}),data:$("#form-execute-step").serialize()}).done(function(data){data.form?($("#form-execute-step").replaceWith($(data.form)),triggerStepFormEventListeners()):(showStepAndCloseStepForm(data),updateBehatScenario(testId))})}function showStepAndCloseStepForm(data){var id=data.id,row=data.row;$(row).insertBefore($("#step-footer")),triggerStepEventListeners(id),resetStepFormAndCloseModal(),swal("Step saved with success !","","success")}function updateStep(id){$.ajax({type:"POST",url:Routing.generate("app_update_application_test_execute_step_ajax",{id:id}),data:$("#form-execute-step").serialize()}).done(function(data){data.form?($("#form-execute-step").replaceWith($(data.form)),triggerStepFormEventListeners()):(updateStepAndCloseStepForm(data),updateBehatScenario(data.testId))})}function updateStepAndCloseStepForm(data){var id=data.id,$row=$(data.row);$("#step-row-"+id).replaceWith($row),triggerStepEventListeners(id),resetStepFormAndCloseModal(),swal("Step updated with success !","","success")}function deleteExecuteStep(id,order,testId){swal({title:"Delete step #"+order+" ?",text:"You will not be able to recover this step !",type:"warning",showCancelButton:!0,confirmButtonColor:"#DD6B55",confirmButtonText:"Yes, delete it !",closeOnConfirm:!1},function(){$.ajax({type:"POST",url:Routing.generate("app_delete_application_test_execute_step_ajax",{id:id})}).done(function(data){data.error?swal("Step #"+order+" not deleted !",data.error,"error"):($("#step-row-"+id).css("visibility","hidden").animate({height:0,width:0},300,function(){$(this).remove(),updateExecuteStepsOrders(testId),updateBehatScenario(testId)}),swal("Step deleted with success !","","success"))})})}function updateExecuteStepsOrders(testId){$.ajax({type:"POST",url:Routing.generate("app_get_application_test_execute_step_orders_ajax",{id:testId})}).done(function(data){jQuery.each(data,function(id,order){$("#step-order-"+id).html(order),$("#delete-execute-step-"+id).data("order",order)})})}function triggerControlStepFormEventListeners(){$("#control_step_page").change(function(){var stepId=$(this).data("step-id");updateControlStepFormAfterPageOrObjectSelection(stepId)}),$("#control_step_object").change(function(){var stepId=$(this).data("step-id");updateControlStepFormAfterPageOrObjectSelection(stepId)}),$("#control_step_action").change(function(){var stepId=$(this).data("step-id");updateControlStepFormAfterActionSelection(stepId)})}function showControlStepFormModal(form,title){$("#modal-control-step-body").html(form),$("#modal-control-step-title").html(title),$("#modal-control-step").modal("show")}function triggerControlStepEventListeners(id){$("#delete-control-step-"+id).click(function(event){event.preventDefault();var id=$(this).data("id"),order=$(this).data("order"),stepId=$(this).data("step-id"),stepOrder=$(this).data("step-order");deleteControlStep(id,order,stepId,stepOrder)}).tooltip(),$("#edit-control-step-"+id).click(function(event){event.preventDefault();var id=$(this).data("id"),stepId=$(this).data("step-id");showEditControlStepForm(id,stepId)}).tooltip()}function resetControlStepFormAndCloseModal(){$("#form-control-step")[0].reset(),$("#control_step_action").parent().remove(),$("#control_step_parameterDatas").parent().remove(),$("#control_step_object").val($("#execute_step_object option:first").val()),$("#modal-control-step").modal("hide")}function showAddControlStepForm(id){$.ajax({type:"POST",url:Routing.generate("app_get_application_test_step_control_step_form_ajax",{stepId:id})}).done(function(data){$("#save-control-step").data("control-step-id",""),$("#save-control-step").data("step-id",id),showControlStepFormModal(data.form,data.modalTitle),triggerControlStepFormEventListeners()})}function showEditControlStepForm(id,stepId){$.ajax({type:"POST",url:Routing.generate("app_get_application_test_step_control_step_form_ajax",{id:id,stepId:stepId})}).done(function(data){$("#save-control-step").data("control-step-id",id),$("#save-control-step").data("step-id",""),showControlStepFormModal(data.form,data.modalTitle),triggerControlStepFormEventListeners()})}function updateControlStepFormAfterPageOrObjectSelection(stepId){$("#control_step_action").parent().remove(),$("#control_step_parameterDatas").parent().remove(),$.ajax({type:"POST",url:Routing.generate("app_check_application_test_step_control_step_ajax",{id:stepId}),data:$("#form-control-step").serialize()}).done(function(data){data.error?swal("Step not updated !",data.error,"error"):($("#form-control-step").replaceWith($(data.form)),$("#control_step_object").change(function(){var stepId=$(this).data("step-id");updateControlStepFormAfterPageOrObjectSelection(stepId)}),$("#control_step_action").change(function(){var stepId=$(this).data("step-id");updateControlStepFormAfterActionSelection(stepId)}))})}function updateControlStepFormAfterActionSelection(stepId){$("#control_step_parameterDatas").parent().remove(),$.ajax({type:"POST",url:Routing.generate("app_check_application_test_step_control_step_ajax",{id:stepId}),data:$("#form-control-step").serialize()}).done(function(data){data.error?swal("Step not updated !",data.error,"error"):data.form&&($("#form-control-step").replaceWith($(data.form)),$("#control_step_object").change(function(){var stepId=$(this).data("step-id");updateControlStepFormAfterPageOrObjectSelection(stepId)}),$("#control_step_action").change(function(){var stepId=$(this).data("step-id");updateControlStepFormAfterActionSelection(stepId)}))})}function saveControlStep(stepId){$.ajax({type:"POST",url:Routing.generate("app_add_application_test_step_control_step_ajax",{id:stepId}),data:$("#form-control-step").serialize()}).done(function(data){data.form?($("#form-control-step").replaceWith($(data.form)),triggerControlStepFormEventListeners()):(showControlStepAndCloseControlStepForm(data,stepId),updateBehatScenario(data.testId))})}function showControlStepAndCloseControlStepForm(data,stepId){var id=data.id,row=data.row;$(row).insertBefore($("#control-step-footer-"+stepId)),triggerControlStepEventListeners(id),resetControlStepFormAndCloseModal(),swal("Step added with success !","","success")}function updateControlStep(id){$.ajax({type:"POST",url:Routing.generate("app_update_application_test_step_control_step_ajax",{id:id}),data:$("#form-control-step").serialize()}).done(function(data){data.form?($("#form-control-step").replaceWith($(data.form)),triggerControlStepFormEventListeners()):(updateControlStepAndCloseControlStepForm(data,id),updateBehatScenario(data.testId))})}function updateControlStepAndCloseControlStepForm(data){var id=data.id,$row=$(data.row);$("#control-step-row-"+id).replaceWith($row),triggerControlStepEventListeners(id),resetControlStepFormAndCloseModal(),swal("Step updated with success !","","success")}function deleteControlStep(id,order,stepId,stepOrder){swal({title:"Delete verification step #"+order+" of step #"+stepOrder+" ?",text:"You will not be able to recover this step !",type:"warning",showCancelButton:!0,confirmButtonColor:"#DD6B55",confirmButtonText:"Yes, delete it !",closeOnConfirm:!1},function(){$.ajax({type:"POST",url:Routing.generate("app_delete_application_test_step_control_step_ajax",{id:id})}).done(function(data){data.error?swal("Verification step #"+order+" of step #"+stepOrder+" not deleted !",data.error,"error"):($("#control-step-row-"+id).css("visibility","hidden").animate({height:0,width:0},300,function(){$(this).remove(),updateControlStepsOrders(stepId),updateBehatScenario(data.testId)
}),swal("Step deleted with success !","","success"))})})}function updateControlStepsOrders(stepId){$.ajax({type:"POST",url:Routing.generate("app_get_application_test_step_control_step_orders_ajax",{id:stepId})}).done(function(data){jQuery.each(data,function(id,order){$("#control-step-order-"+id).html(order),$("#delete-control-step-"+id).data("order",order)})})}function showPrerequisiteFormModal(form){$("#modal-prerequisite-body").html(form),$("#modal-prerequisite").modal("show")}function showAddPrerequisiteForm(id){$.ajax({type:"POST",url:Routing.generate("app_get_application_test_prerequisite_form_ajax",{id:id})}).done(function(data){$("#save-prerequisite").data("test-id",id),showPrerequisiteFormModal(data.form)})}function triggerPrerequisiteEventListeners(id){$("#delete-prerequisite-"+id).click(function(event){event.preventDefault();var id=$(this).data("id"),name=$(this).data("name");deletePrerequisite(id,name)}).tooltip()}function resetPrerequisiteFormAndCloseModal(){$("#form-prerequisite")[0].reset(),$("#prerequisite_test").val($("#prerequisite_test option:first").val()),$("#modal-prerequisite").modal("hide")}function savePrerequisite(testId){$.ajax({type:"POST",url:Routing.generate("app_add_application_test_prerequisite_ajax",{id:testId}),data:$("#form-prerequisite").serialize()}).done(function(data){data.form?$("#form-prerequisite").replaceWith($(data.form)):(showPrerequisiteAndClosePrerequisiteForm(data),updatePrerequisitesCount(),data.resetStartingPage?($("#starting-page-name").text(""),$("#form_startingPage").val($("#form_startingPage option:first").val())):($("#starting-page-name").html(data.startingPage.name),disableStartingPageSelection()),updateBehatScenario(testId))})}function showPrerequisiteAndClosePrerequisiteForm(data){var id=data.id,li=data.li;$(li).insertBefore($("#add-prerequisite").parent()),triggerPrerequisiteEventListeners(id),resetPrerequisiteFormAndCloseModal(),swal("Prerequisite added with success !","","success")}function deletePrerequisite(id,name){swal({title:"Delete prerequisite "+name+" ?",text:"You will not be able to recover this prerequisite !",type:"warning",showCancelButton:!0,confirmButtonColor:"#DD6B55",confirmButtonText:"Yes, delete it !",closeOnConfirm:!1},function(){$.ajax({type:"POST",url:Routing.generate("app_delete_application_test_prerequisite_ajax",{id:id})}).done(function(data){if(data.error)swal("Prerequisite "+name+" not deleted !",data.error,"error");else{var testId=data.testId;$("#item-prerequisite-"+id).css("visibility","hidden").animate({height:0,width:0},300,function(){$(this).remove(),updatePrerequisitesCount(),updatePrerequisitesOrders(testId),data.startingPage?$("#starting-page-name").html(data.startingPage.name):data.resetStartingPage&&($("#starting-page-name").text(""),$("#form_startingPage").val($("#form_startingPage option:first").val())),updateBehatScenario(testId)}),swal("Prerequisite deleted with success !","","success")}})})}function updatePrerequisitesCount(){var count=$("[id^=item-prerequisite-]").length;$("#prerequisites-count").html(count),0===count&&enableStartingPageSelection()}function updatePrerequisitesOrders(testId){$.ajax({type:"POST",url:Routing.generate("app_get_application_test_prerequisites_orders_ajax",{id:testId})}).done(function(data){jQuery.each(data,function(id,order){$("#prerequisite-order-"+id).html(order)})})}function updateBehatScenario(testId){$.ajax({type:"POST",url:Routing.generate("app_get_application_test_behat_scenario_ajax",{id:testId})}).done(function(data){$("#behat-scenario").html(data.scenario)})}function refreshTestFolderSubtitle(count){var subtitle="";subtitle+=1>=count?"There is ":"There are ",subtitle+=0===count?"no ":'<span class="badge">'+count+"</span>",subtitle+=" test folder"+(count>1?"s":""),$("#test-folders-count").html(subtitle)}function saveTestFolder(applicationId){var testsTreeHtmlId="#tree-tests-"+applicationId,selectedFolder=$(testsTreeHtmlId).treeview("getSelected"),parentFolderId=-1;if(1===selectedFolder.length){var parentFolder=selectedFolder[0],href=parentFolder.href,id=href.substring(href.lastIndexOf("-")+1),type=href.substring(href.indexOf("-")+1,href.lastIndexOf("-"));"folder"===type&&(parentFolderId=id)}$.ajax({type:"POST",url:Routing.generate("app_add_application_test_folder_ajax",{id:applicationId,parentId:parentFolderId}),data:$("#form-add-test-folder").serialize()}).done(function(data){if(data.error)swal("Test folder not added !",data.error,"error");else{var name=(data.id,data.name),testFoldersCount=data.testFoldersCount;hideTestsPanel(applicationId),$(testsTreeHtmlId).treeview({data:data.treeTests,showBorder:!1,showCheckbox:!0,onNodeSelected:function(event,data){showEntityProperties(data)},onNodeUnselected:function(){hideEntityPropertiesPanelBodyAndFooter(),clearAddTestDataAttributes(),clearActionsHref()},onNodeChecked:function(event,data){checkTreeChildNodes(testsTreeHtmlId,data),refreshCheckedTestsTreeEntitiesCount(applicationId)},onNodeUnchecked:function(event,data){uncheckTreeChildNodes(testsTreeHtmlId,data),uncheckTreeParentNode(testsTreeHtmlId,data),refreshCheckedTestsTreeEntitiesCount(applicationId)}}),$(testsTreeHtmlId).treeview("collapseAll",{silent:!0}),showTestsPanel(applicationId);var selectedNode=$(testsTreeHtmlId).treeview("getSelected")[0];showEntityProperties(selectedNode),$(testsTreeHtmlId).treeview("revealNode",selectedNode),refreshTestFolderSubtitle(testFoldersCount,applicationId),swal(name+" added !","Your folder has been added.","success"),$("#form-add-test-folder")[0].reset(),$("#modal-add-test-folder").modal("hide")}})}function refreshTestSubtitle(count){var subtitle="";subtitle+=1>=count?"There is ":"There are ",subtitle+=0===count?"no ":'<span class="badge">'+count+"</span>",subtitle+=" test"+(count>1?"s":""),$("#tests-count").html(subtitle)}function refreshTestsTree(id,collapse,checkbox){$.ajax({type:"POST",url:Routing.generate("app_application_get_tests_tree_ajax",{id:id})}).done(function(data){showTestsTree(data,collapse,checkbox),revealTestsTreeSelectedNode(id)})}function showTestsTree(data,collapse,checkbox){jQuery.each(data,function(i,val){var id=i.substring(i.lastIndexOf("-")+1),htmlId="#"+i;$(htmlId).treeview({data:val,showBorder:!1,showCheckbox:checkbox,onNodeSelected:function(event,data){showEntityProperties(data)},onNodeUnselected:function(){hideEntityPropertiesPanelBodyAndFooter(),clearAddTestDataAttributes(),clearActionsHref()},onNodeChecked:function(event,data){checkTreeChildNodes(htmlId,data),refreshCheckedTestsTreeEntitiesCount(id)},onNodeUnchecked:function(event,data){uncheckTreeChildNodes(htmlId,data),uncheckTreeParentNode(htmlId,data),refreshCheckedTestsTreeEntitiesCount(id)}}),collapse&&$(htmlId).treeview("collapseAll",{silent:!0}),revealTestsTreeSelectedNode(id),showTestsPanel(id)})}function showTestsTreeWithSelectedFolder(testFolderId){$.ajax({type:"POST",url:Routing.generate("app_application_get_tests_tree_with_selected_folder_ajax",{id:testFolderId})}).done(function(data){showTestsTree(data,!0,!0)})}function showTestsTreeWithSelectedTest(testId){$.ajax({type:"POST",url:Routing.generate("app_application_get_tests_tree_with_selected_test_ajax",{id:testId})}).done(function(data){showTestsTree(data,!0,!0)})}function revealTestsTreeSelectedNode(applicationId){var testsTreeHtmlId="#tree-tests-"+applicationId,selectedNode=$(testsTreeHtmlId).treeview("getSelected")[0];selectedNode&&$(testsTreeHtmlId).treeview("revealNode",selectedNode)}function hideTestsPanel(applicationId){var testsTreeHtmlId="#tree-tests-"+applicationId;$(testsTreeHtmlId+"-loader").addClass("three-quarters-loader"),$(testsTreeHtmlId).hide()}function showTestsPanel(applicationId){var testsTreeHtmlId="#tree-tests-"+applicationId;$(testsTreeHtmlId+"-loader").removeClass("three-quarters-loader"),$(testsTreeHtmlId).show()}function setAddTestDataAttributes(id,name,description){$("#add-test").data("test-folder-id",id),$("#add-test").data("test-folder-name",name),$("#add-test").data("test-folder-description",description)}function clearAddTestDataAttributes(){$("#add-test").removeData("test-folder-id"),$("#add-test").removeData("test-folder-name"),$("#add-test").removeData("test-folder-description")}function showAddTestForm(id,name,description){updateAddTestModalTitle(name,description),$("#save-test").data("test-folder-id",id),$("#save-test").data("test-folder-name",name),$("#modal-add-test").modal("show")}function saveTest(testFolderId,testFolderName){$.ajax({type:"POST",url:Routing.generate("app_add_application_test_folder_test_ajax",{id:testFolderId}),data:$("#form-add-test").serialize()}).done(function(data){if(data.error)swal("Test not added !",data.error,"error");else{var name=(data.id,data.name),testsCount=data.testsCount,applicationId=data.applicationId,testsTreeHtmlId="#tree-tests-"+applicationId;hideTestsPanel(applicationId),$(testsTreeHtmlId).treeview({data:data.treeTests,showBorder:!1,showCheckbox:!0,onNodeSelected:function(event,data){showEntityProperties(data)},onNodeUnselected:function(){hideEntityPropertiesPanelBodyAndFooter(),clearAddTestDataAttributes(),clearActionsHref()},onNodeChecked:function(event,data){checkTreeChildNodes(testsTreeHtmlId,data),refreshCheckedTestsTreeEntitiesCount(applicationId)},onNodeUnchecked:function(event,data){uncheckTreeChildNodes(testsTreeHtmlId,data),uncheckTreeParentNode(testsTreeHtmlId,data),refreshCheckedTestsTreeEntitiesCount(applicationId)}}),$(testsTreeHtmlId).treeview("collapseAll",{silent:!0}),showTestsPanel(applicationId);var selectedNode=$(testsTreeHtmlId).treeview("getSelected")[0];showEntityProperties(selectedNode),$(testsTreeHtmlId).treeview("revealNode",selectedNode),$(testsTreeHtmlId).treeview("expandNode",selectedNode),refreshTestSubtitle(testsCount,applicationId),swal(name+" added to "+testFolderName+" !","Your test has been added.","success"),$("#form-add-test")[0].reset(),$("#modal-add-test").modal("hide")}})}function updateAddTestModalTitle(name,description){$("#new-test-test-folder-name").html(name),$("#new-test-test-folder-description").html(description)}function showEntityProperties(treeNode){if(treeNode){$("#panel-body-entity-properties").show(),$("#panel-footer-entity-properties").show(),$("#entity-properties-loader").show();var href=treeNode.href,id=href.substring(href.lastIndexOf("-")+1),type=href.substring(href.indexOf("-")+1,href.lastIndexOf("-"));switch($("#entity-icon").removeClass("fontello-icon-puzzle"),type){case"folder":hideEntityActions(),$("#entity-icon").removeClass().addClass("fontello-icon-folder"),$.ajax({type:"POST",url:Routing.generate("app_application_get_test_folder_ajax",{id:id})}).done(function(data){$("#entity-properties-loader").hide();var folder=data.testFolder,name=folder.name,description=folder.description,createdAt=folder.createdAt;$("#entity-name").editable("option","pk",id),$("#entity-name").editable("option","url",Routing.generate("app_application_test_folder_update_name_ajax",{id:id})),$("#entity-description").editable("option","pk",id),$("#entity-description").editable("option","url",Routing.generate("app_application_test_folder_update_description_ajax",{id:id})),$("#entity-name").editable("setValue",name,!1),$("#entity-description").editable("setValue",description,!1),$("#entity-creation-date").html(createdAt),setAddTestDataAttributes(id,name,description)});break;case"test":showEntityActions(),$("#edit-entity").attr("href",Routing.generate("app_index_application_test_editor",{id:id})),$("#entity-icon").removeClass().addClass("fontello-icon-tasks"),$.ajax({type:"POST",url:Routing.generate("app_application_get_test_folder_test_ajax",{id:id})}).done(function(data){$("#entity-properties-loader").hide();var test=data.test,name=test.name,description=test.description,createdAt=test.createdAt;$("#entity-name").editable("option","pk",id),$("#entity-name").editable("option","url",Routing.generate("app_application_test_folder_test_update_name_ajax",{id:id})),$("#entity-description").editable("option","pk",id),$("#entity-description").editable("option","url",Routing.generate("app_application_test_folder_test_update_description_ajax",{id:id})),$("#entity-name").editable("setValue",name,!1),$("#entity-description").editable("setValue",description,!1),$("#entity-creation-date").html(createdAt)});break;default:$("#entity-icon").removeClass().addClass("fontello-icon-puzzle")}}else hideEntityPropertiesPanelBodyAndFooter()}function hideEntityPropertiesPanelBodyAndFooter(){$("#panel-body-entity-properties").hide(),$("#panel-footer-entity-properties").hide()}function refreshCheckedTestsTreeEntitiesCount(applicationId){var checkedEntities=$("#tree-tests-"+applicationId).treeview("getChecked"),checkedEntitiesCount=checkedEntities.length;$("#checked-entities-count").html(checkedEntitiesCount),checkedEntitiesCount>0?$("#delete-checked-entities").removeClass("disabled"):$("#delete-checked-entities").addClass("disabled")}function deleteEntities(applicationId){var testsTreeHtmlId="#tree-tests-"+applicationId;swal({title:"Delete selected entities ?",text:"You will not be able to recover them !",type:"warning",showCancelButton:!0,confirmButtonColor:"#DD6B55",confirmButtonText:"Yes, delete them !",closeOnConfirm:!1},function(){var objects=$(testsTreeHtmlId).treeview("getChecked");$.ajax({type:"POST",url:Routing.generate("app_application_test_entities_delete_ajax",{id:applicationId}),data:{objects:objects,selectedNode:$(testsTreeHtmlId).treeview("getSelected")[0]}}).done(function(data){if(data.error)swal("Selected entities were not deleted !",data.error,"error");else{var count=data.count,testFoldersCount=data.testFoldersCount,testsCount=data.testsCount,applicationId=data.applicationId;hideTestsPanel(applicationId),$(testsTreeHtmlId).treeview({data:data.treeTests,showBorder:!1,showCheckbox:!0,onNodeSelected:function(event,data){showEntityProperties(data)},onNodeUnselected:function(){hideEntityPropertiesPanelBodyAndFooter(),clearAddTestDataAttributes(),clearActionsHref()},onNodeChecked:function(event,data){checkTreeChildNodes(testsTreeHtmlId,data),refreshCheckedTestsTreeEntitiesCount(applicationId)},onNodeUnchecked:function(event,data){uncheckTreeChildNodes(testsTreeHtmlId,data),uncheckTreeParentNode(testsTreeHtmlId,data),refreshCheckedTestsTreeEntitiesCount(applicationId)}}),$(testsTreeHtmlId).treeview("collapseAll",{silent:!0}),showTestsPanel(applicationId),refreshTestSubtitle(testsCount,applicationId),refreshTestFolderSubtitle(testFoldersCount,applicationId),refreshCheckedTestsTreeEntitiesCount(applicationId),swal("Selected entities deleted !","You have deleted "+count+" entit"+(count>1?"ies":"y"),"success")}})})}function hideEntityActions(){$("#panel-body-content-entity-actions").hide()}function showEntityActions(){$("#panel-body-content-entity-actions").show()}function clearActionsHref(){$("#edit-entity").removeAttr("href")}!function(window,undefined){function isArraylike(obj){var length=obj.length,type=jQuery.type(obj);return jQuery.isWindow(obj)?!1:1===obj.nodeType&&length?!0:"array"===type||"function"!==type&&(0===length||"number"==typeof length&&length>0&&length-1 in obj)}function createOptions(options){var object=optionsCache[options]={};return jQuery.each(options.match(core_rnotwhite)||[],function(_,flag){object[flag]=!0}),object}function Data(){Object.defineProperty(this.cache={},0,{get:function(){return{}}}),this.expando=jQuery.expando+Math.random()}function dataAttr(elem,key,data){var name;if(data===undefined&&1===elem.nodeType)if(name="data-"+key.replace(rmultiDash,"-$1").toLowerCase(),data=elem.getAttribute(name),"string"==typeof data){try{data="true"===data?!0:"false"===data?!1:"null"===data?null:+data+""===data?+data:rbrace.test(data)?JSON.parse(data):data}catch(e){}data_user.set(elem,key,data)}else data=undefined;return data}function returnTrue(){return!0}function returnFalse(){return!1}function safeActiveElement(){try{return document.activeElement}catch(err){}}function sibling(cur,dir){for(;(cur=cur[dir])&&1!==cur.nodeType;);return cur}function winnow(elements,qualifier,not){if(jQuery.isFunction(qualifier))return jQuery.grep(elements,function(elem,i){return!!qualifier.call(elem,i,elem)!==not});if(qualifier.nodeType)return jQuery.grep(elements,function(elem){return elem===qualifier!==not});if("string"==typeof qualifier){if(isSimple.test(qualifier))return jQuery.filter(qualifier,elements,not);qualifier=jQuery.filter(qualifier,elements)}return jQuery.grep(elements,function(elem){return core_indexOf.call(qualifier,elem)>=0!==not})}function manipulationTarget(elem,content){return jQuery.nodeName(elem,"table")&&jQuery.nodeName(1===content.nodeType?content:content.firstChild,"tr")?elem.getElementsByTagName("tbody")[0]||elem.appendChild(elem.ownerDocument.createElement("tbody")):elem}function disableScript(elem){return elem.type=(null!==elem.getAttribute("type"))+"/"+elem.type,elem}function restoreScript(elem){var match=rscriptTypeMasked.exec(elem.type);return match?elem.type=match[1]:elem.removeAttribute("type"),elem}function setGlobalEval(elems,refElements){for(var l=elems.length,i=0;l>i;i++)data_priv.set(elems[i],"globalEval",!refElements||data_priv.get(refElements[i],"globalEval"))}function cloneCopyEvent(src,dest){var i,l,type,pdataOld,pdataCur,udataOld,udataCur,events;if(1===dest.nodeType){if(data_priv.hasData(src)&&(pdataOld=data_priv.access(src),pdataCur=data_priv.set(dest,pdataOld),events=pdataOld.events)){delete pdataCur.handle,pdataCur.events={};for(type in events)for(i=0,l=events[type].length;l>i;i++)jQuery.event.add(dest,type,events[type][i])}data_user.hasData(src)&&(udataOld=data_user.access(src),udataCur=jQuery.extend({},udataOld),data_user.set(dest,udataCur))}}function getAll(context,tag){var ret=context.getElementsByTagName?context.getElementsByTagName(tag||"*"):context.querySelectorAll?context.querySelectorAll(tag||"*"):[];return tag===undefined||tag&&jQuery.nodeName(context,tag)?jQuery.merge([context],ret):ret}function fixInput(src,dest){var nodeName=dest.nodeName.toLowerCase();"input"===nodeName&&manipulation_rcheckableType.test(src.type)?dest.checked=src.checked:("input"===nodeName||"textarea"===nodeName)&&(dest.defaultValue=src.defaultValue)}function vendorPropName(style,name){if(name in style)return name;for(var capName=name.charAt(0).toUpperCase()+name.slice(1),origName=name,i=cssPrefixes.length;i--;)if(name=cssPrefixes[i]+capName,name in style)return name;return origName}function isHidden(elem,el){return elem=el||elem,"none"===jQuery.css(elem,"display")||!jQuery.contains(elem.ownerDocument,elem)}function getStyles(elem){return window.getComputedStyle(elem,null)}function showHide(elements,show){for(var display,elem,hidden,values=[],index=0,length=elements.length;length>index;index++)elem=elements[index],elem.style&&(values[index]=data_priv.get(elem,"olddisplay"),display=elem.style.display,show?(values[index]||"none"!==display||(elem.style.display=""),""===elem.style.display&&isHidden(elem)&&(values[index]=data_priv.access(elem,"olddisplay",css_defaultDisplay(elem.nodeName)))):values[index]||(hidden=isHidden(elem),(display&&"none"!==display||!hidden)&&data_priv.set(elem,"olddisplay",hidden?display:jQuery.css(elem,"display"))));for(index=0;length>index;index++)elem=elements[index],elem.style&&(show&&"none"!==elem.style.display&&""!==elem.style.display||(elem.style.display=show?values[index]||"":"none"));return elements}function setPositiveNumber(elem,value,subtract){var matches=rnumsplit.exec(value);return matches?Math.max(0,matches[1]-(subtract||0))+(matches[2]||"px"):value}function augmentWidthOrHeight(elem,name,extra,isBorderBox,styles){for(var i=extra===(isBorderBox?"border":"content")?4:"width"===name?1:0,val=0;4>i;i+=2)"margin"===extra&&(val+=jQuery.css(elem,extra+cssExpand[i],!0,styles)),isBorderBox?("content"===extra&&(val-=jQuery.css(elem,"padding"+cssExpand[i],!0,styles)),"margin"!==extra&&(val-=jQuery.css(elem,"border"+cssExpand[i]+"Width",!0,styles))):(val+=jQuery.css(elem,"padding"+cssExpand[i],!0,styles),"padding"!==extra&&(val+=jQuery.css(elem,"border"+cssExpand[i]+"Width",!0,styles)));return val}function getWidthOrHeight(elem,name,extra){var valueIsBorderBox=!0,val="width"===name?elem.offsetWidth:elem.offsetHeight,styles=getStyles(elem),isBorderBox=jQuery.support.boxSizing&&"border-box"===jQuery.css(elem,"boxSizing",!1,styles);if(0>=val||null==val){if(val=curCSS(elem,name,styles),(0>val||null==val)&&(val=elem.style[name]),rnumnonpx.test(val))return val;valueIsBorderBox=isBorderBox&&(jQuery.support.boxSizingReliable||val===elem.style[name]),val=parseFloat(val)||0}return val+augmentWidthOrHeight(elem,name,extra||(isBorderBox?"border":"content"),valueIsBorderBox,styles)+"px"}function css_defaultDisplay(nodeName){var doc=document,display=elemdisplay[nodeName];return display||(display=actualDisplay(nodeName,doc),"none"!==display&&display||(iframe=(iframe||jQuery("<iframe frameborder='0' width='0' height='0'/>").css("cssText","display:block !important")).appendTo(doc.documentElement),doc=(iframe[0].contentWindow||iframe[0].contentDocument).document,doc.write("<!doctype html><html><body>"),doc.close(),display=actualDisplay(nodeName,doc),iframe.detach()),elemdisplay[nodeName]=display),display}function actualDisplay(name,doc){var elem=jQuery(doc.createElement(name)).appendTo(doc.body),display=jQuery.css(elem[0],"display");return elem.remove(),display}function buildParams(prefix,obj,traditional,add){var name;if(jQuery.isArray(obj))jQuery.each(obj,function(i,v){traditional||rbracket.test(prefix)?add(prefix,v):buildParams(prefix+"["+("object"==typeof v?i:"")+"]",v,traditional,add)});else if(traditional||"object"!==jQuery.type(obj))add(prefix,obj);else for(name in obj)buildParams(prefix+"["+name+"]",obj[name],traditional,add)}function addToPrefiltersOrTransports(structure){return function(dataTypeExpression,func){"string"!=typeof dataTypeExpression&&(func=dataTypeExpression,dataTypeExpression="*");var dataType,i=0,dataTypes=dataTypeExpression.toLowerCase().match(core_rnotwhite)||[];if(jQuery.isFunction(func))for(;dataType=dataTypes[i++];)"+"===dataType[0]?(dataType=dataType.slice(1)||"*",(structure[dataType]=structure[dataType]||[]).unshift(func)):(structure[dataType]=structure[dataType]||[]).push(func)}}function inspectPrefiltersOrTransports(structure,options,originalOptions,jqXHR){function inspect(dataType){var selected;return inspected[dataType]=!0,jQuery.each(structure[dataType]||[],function(_,prefilterOrFactory){var dataTypeOrTransport=prefilterOrFactory(options,originalOptions,jqXHR);return"string"!=typeof dataTypeOrTransport||seekingTransport||inspected[dataTypeOrTransport]?seekingTransport?!(selected=dataTypeOrTransport):void 0:(options.dataTypes.unshift(dataTypeOrTransport),inspect(dataTypeOrTransport),!1)}),selected}var inspected={},seekingTransport=structure===transports;return inspect(options.dataTypes[0])||!inspected["*"]&&inspect("*")}function ajaxExtend(target,src){var key,deep,flatOptions=jQuery.ajaxSettings.flatOptions||{};for(key in src)src[key]!==undefined&&((flatOptions[key]?target:deep||(deep={}))[key]=src[key]);return deep&&jQuery.extend(!0,target,deep),target}function ajaxHandleResponses(s,jqXHR,responses){for(var ct,type,finalDataType,firstDataType,contents=s.contents,dataTypes=s.dataTypes;"*"===dataTypes[0];)dataTypes.shift(),ct===undefined&&(ct=s.mimeType||jqXHR.getResponseHeader("Content-Type"));if(ct)for(type in contents)if(contents[type]&&contents[type].test(ct)){dataTypes.unshift(type);break}if(dataTypes[0]in responses)finalDataType=dataTypes[0];else{for(type in responses){if(!dataTypes[0]||s.converters[type+" "+dataTypes[0]]){finalDataType=type;break}firstDataType||(firstDataType=type)}finalDataType=finalDataType||firstDataType}return finalDataType?(finalDataType!==dataTypes[0]&&dataTypes.unshift(finalDataType),responses[finalDataType]):void 0}function ajaxConvert(s,response,jqXHR,isSuccess){var conv2,current,conv,tmp,prev,converters={},dataTypes=s.dataTypes.slice();if(dataTypes[1])for(conv in s.converters)converters[conv.toLowerCase()]=s.converters[conv];for(current=dataTypes.shift();current;)if(s.responseFields[current]&&(jqXHR[s.responseFields[current]]=response),!prev&&isSuccess&&s.dataFilter&&(response=s.dataFilter(response,s.dataType)),prev=current,current=dataTypes.shift())if("*"===current)current=prev;else if("*"!==prev&&prev!==current){if(conv=converters[prev+" "+current]||converters["* "+current],!conv)for(conv2 in converters)if(tmp=conv2.split(" "),tmp[1]===current&&(conv=converters[prev+" "+tmp[0]]||converters["* "+tmp[0]])){conv===!0?conv=converters[conv2]:converters[conv2]!==!0&&(current=tmp[0],dataTypes.unshift(tmp[1]));break}if(conv!==!0)if(conv&&s["throws"])response=conv(response);else try{response=conv(response)}catch(e){return{state:"parsererror",error:conv?e:"No conversion from "+prev+" to "+current}}}return{state:"success",data:response}}function createFxNow(){return setTimeout(function(){fxNow=undefined}),fxNow=jQuery.now()}function createTween(value,prop,animation){for(var tween,collection=(tweeners[prop]||[]).concat(tweeners["*"]),index=0,length=collection.length;length>index;index++)if(tween=collection[index].call(animation,prop,value))return tween}function Animation(elem,properties,options){var result,stopped,index=0,length=animationPrefilters.length,deferred=jQuery.Deferred().always(function(){delete tick.elem}),tick=function(){if(stopped)return!1;for(var currentTime=fxNow||createFxNow(),remaining=Math.max(0,animation.startTime+animation.duration-currentTime),temp=remaining/animation.duration||0,percent=1-temp,index=0,length=animation.tweens.length;length>index;index++)animation.tweens[index].run(percent);return deferred.notifyWith(elem,[animation,percent,remaining]),1>percent&&length?remaining:(deferred.resolveWith(elem,[animation]),!1)},animation=deferred.promise({elem:elem,props:jQuery.extend({},properties),opts:jQuery.extend(!0,{specialEasing:{}},options),originalProperties:properties,originalOptions:options,startTime:fxNow||createFxNow(),duration:options.duration,tweens:[],createTween:function(prop,end){var tween=jQuery.Tween(elem,animation.opts,prop,end,animation.opts.specialEasing[prop]||animation.opts.easing);return animation.tweens.push(tween),tween},stop:function(gotoEnd){var index=0,length=gotoEnd?animation.tweens.length:0;if(stopped)return this;for(stopped=!0;length>index;index++)animation.tweens[index].run(1);return gotoEnd?deferred.resolveWith(elem,[animation,gotoEnd]):deferred.rejectWith(elem,[animation,gotoEnd]),this}}),props=animation.props;for(propFilter(props,animation.opts.specialEasing);length>index;index++)if(result=animationPrefilters[index].call(animation,elem,props,animation.opts))return result;return jQuery.map(props,createTween,animation),jQuery.isFunction(animation.opts.start)&&animation.opts.start.call(elem,animation),jQuery.fx.timer(jQuery.extend(tick,{elem:elem,anim:animation,queue:animation.opts.queue})),animation.progress(animation.opts.progress).done(animation.opts.done,animation.opts.complete).fail(animation.opts.fail).always(animation.opts.always)}function propFilter(props,specialEasing){var index,name,easing,value,hooks;for(index in props)if(name=jQuery.camelCase(index),easing=specialEasing[name],value=props[index],jQuery.isArray(value)&&(easing=value[1],value=props[index]=value[0]),index!==name&&(props[name]=value,delete props[index]),hooks=jQuery.cssHooks[name],hooks&&"expand"in hooks){value=hooks.expand(value),delete props[name];for(index in value)index in props||(props[index]=value[index],specialEasing[index]=easing)}else specialEasing[name]=easing}function defaultPrefilter(elem,props,opts){var prop,value,toggle,tween,hooks,oldfire,anim=this,orig={},style=elem.style,hidden=elem.nodeType&&isHidden(elem),dataShow=data_priv.get(elem,"fxshow");opts.queue||(hooks=jQuery._queueHooks(elem,"fx"),null==hooks.unqueued&&(hooks.unqueued=0,oldfire=hooks.empty.fire,hooks.empty.fire=function(){hooks.unqueued||oldfire()}),hooks.unqueued++,anim.always(function(){anim.always(function(){hooks.unqueued--,jQuery.queue(elem,"fx").length||hooks.empty.fire()})})),1===elem.nodeType&&("height"in props||"width"in props)&&(opts.overflow=[style.overflow,style.overflowX,style.overflowY],"inline"===jQuery.css(elem,"display")&&"none"===jQuery.css(elem,"float")&&(style.display="inline-block")),opts.overflow&&(style.overflow="hidden",anim.always(function(){style.overflow=opts.overflow[0],style.overflowX=opts.overflow[1],style.overflowY=opts.overflow[2]}));for(prop in props)if(value=props[prop],rfxtypes.exec(value)){if(delete props[prop],toggle=toggle||"toggle"===value,value===(hidden?"hide":"show")){if("show"!==value||!dataShow||dataShow[prop]===undefined)continue;hidden=!0}orig[prop]=dataShow&&dataShow[prop]||jQuery.style(elem,prop)}if(!jQuery.isEmptyObject(orig)){dataShow?"hidden"in dataShow&&(hidden=dataShow.hidden):dataShow=data_priv.access(elem,"fxshow",{}),toggle&&(dataShow.hidden=!hidden),hidden?jQuery(elem).show():anim.done(function(){jQuery(elem).hide()}),anim.done(function(){var prop;data_priv.remove(elem,"fxshow");for(prop in orig)jQuery.style(elem,prop,orig[prop])});for(prop in orig)tween=createTween(hidden?dataShow[prop]:0,prop,anim),prop in dataShow||(dataShow[prop]=tween.start,hidden&&(tween.end=tween.start,tween.start="width"===prop||"height"===prop?1:0))}}function Tween(elem,options,prop,end,easing){return new Tween.prototype.init(elem,options,prop,end,easing)}function genFx(type,includeWidth){var which,attrs={height:type},i=0;for(includeWidth=includeWidth?1:0;4>i;i+=2-includeWidth)which=cssExpand[i],attrs["margin"+which]=attrs["padding"+which]=type;return includeWidth&&(attrs.opacity=attrs.width=type),attrs}function getWindow(elem){return jQuery.isWindow(elem)?elem:9===elem.nodeType&&elem.defaultView}var rootjQuery,readyList,core_strundefined=typeof undefined,location=window.location,document=window.document,docElem=document.documentElement,_jQuery=window.jQuery,_$=window.$,class2type={},core_deletedIds=[],core_version="2.0.1",core_concat=core_deletedIds.concat,core_push=core_deletedIds.push,core_slice=core_deletedIds.slice,core_indexOf=core_deletedIds.indexOf,core_toString=class2type.toString,core_hasOwn=class2type.hasOwnProperty,core_trim=core_version.trim,jQuery=function(selector,context){return new jQuery.fn.init(selector,context,rootjQuery)},core_pnum=/[+-]?(?:\d*\.|)\d+(?:[eE][+-]?\d+|)/.source,core_rnotwhite=/\S+/g,rquickExpr=/^(?:\s*(<[\w\W]+>)[^>]*|#([\w-]*))$/,rsingleTag=/^<(\w+)\s*\/?>(?:<\/\1>|)$/,rmsPrefix=/^-ms-/,rdashAlpha=/-([\da-z])/gi,fcamelCase=function(all,letter){return letter.toUpperCase()},completed=function(){document.removeEventListener("DOMContentLoaded",completed,!1),window.removeEventListener("load",completed,!1),jQuery.ready()};jQuery.fn=jQuery.prototype={jquery:core_version,constructor:jQuery,init:function(selector,context,rootjQuery){var match,elem;if(!selector)return this;if("string"==typeof selector){if(match="<"===selector.charAt(0)&&">"===selector.charAt(selector.length-1)&&selector.length>=3?[null,selector,null]:rquickExpr.exec(selector),!match||!match[1]&&context)return!context||context.jquery?(context||rootjQuery).find(selector):this.constructor(context).find(selector);if(match[1]){if(context=context instanceof jQuery?context[0]:context,jQuery.merge(this,jQuery.parseHTML(match[1],context&&context.nodeType?context.ownerDocument||context:document,!0)),rsingleTag.test(match[1])&&jQuery.isPlainObject(context))for(match in context)jQuery.isFunction(this[match])?this[match](context[match]):this.attr(match,context[match]);return this}return elem=document.getElementById(match[2]),elem&&elem.parentNode&&(this.length=1,this[0]=elem),this.context=document,this.selector=selector,this}return selector.nodeType?(this.context=this[0]=selector,this.length=1,this):jQuery.isFunction(selector)?rootjQuery.ready(selector):(selector.selector!==undefined&&(this.selector=selector.selector,this.context=selector.context),jQuery.makeArray(selector,this))},selector:"",length:0,toArray:function(){return core_slice.call(this)},get:function(num){return null==num?this.toArray():0>num?this[this.length+num]:this[num]
