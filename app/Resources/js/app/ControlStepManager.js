var ControlStepManager = {
    init: function() {
        ...
        $("#save-control-step").click(function() {
            ...
            if (parentStepId !== '') {
                ControlStepManager.save(parentStepId);
            }
            var stepId = $(this).data('control-step-id');
            if (stepId !== '') {
                ControlStepManager.update(stepId);
            }
        });
    ...
    openAddFormModal: function(stepId) {
        $.ajax({
            ...
            ControlStepManager.triggerFormEventListeners();
        });
    },
    ...
    add: function(id, stepId, row) {
        ...
        ControlStepManager.initItem(id);
    },
    remove: function(id, stepId, testId) {
        $('#control-step-row-' + id).css('visibility', 'hidden').animate({
            ...
            $(this).remove();
            ControlStepManager.refreshOrders(stepId);
    ...
    save: function(stepId) {
        $.ajax({
            ...
            if (data.form) {
                ...
                ControlStepManager.add(data.id, stepId, data.row);
                ControlStepManager.closeFormModal();
                ControlStepManager.resetForm();
    ...
    update: function(id) {
        $.ajax({
            ...
            if (data.form) {
                ...
                ControlStepManager.refresh(data.id, data.row);
                ControlStepManager.closeFormModal();
                ControlStepManager.resetForm();
                TestEditor.refreshBehatScenario(data.testId);
        ...
        function(){
            $.ajax({
                ...
                if (data.error) {
                    ...
                    ControlStepManager.remove(id, stepId, data.testId);
